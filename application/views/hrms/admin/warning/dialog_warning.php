...
<?php echo form_open('hrms/admin/warning/update/'.$warning_id, $attributes, $hidden);?>
		...
		$.ajax({
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					"bDestroy": true,
					"ajax": {
						url : "<?php echo site_url("hrms/admin/warning/warning_list") ?>",
						type : 'GET'
					},
					dom: 'lBfrtip',
    ...
    <tbody>
      ...
      <tr>
        ...
        <td style="display: table-cell;"><?php if($attachment!='' && $attachment!='no file') {?>
          <img src="<?php echo base_url().'uploads/warning/'.$attachment;?>" width="70px" id="u_file">&nbsp; <a href="<?php echo site_url()?>hrms/admin/download?type=warning&filename=<?php echo $attachment;?>"><?php echo $this->lang->line('xin_download');?></a>
          <?php } ?></td>
      </tr>
      <tr>
