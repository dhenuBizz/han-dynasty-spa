...
class Complaints extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('19',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/complaints/complaint_list", $data, TRUE);
    ...
    public function complaint_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/complaints/complaint_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function read()
	{
		...
		$data = array(
			...
			$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/complaints/dialog_complaint', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
	...
	// get company > employees
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/complaints/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function get_complaint_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/complaints/get_complaint_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
