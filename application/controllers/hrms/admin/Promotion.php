	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('18',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/promotion/promotion_list", $data, TRUE);
    ...
    public function promotion_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/promotion/promotion_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/promotion/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function get_employee_designations() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/promotion/get_designations", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function read() {
		...
		$data = array(
			...
			$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/promotion/dialog_promotion', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
