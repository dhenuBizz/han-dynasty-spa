   ...
   /*Function to set JSON output*/
	...
	public function hr() {
		...
		if(in_array('95',$role_resources_ids)) {
			$data['subview'] = $this->load->view("hrms/admin/calendar/calendar_hr", $data, TRUE);
	...
	public function add_cal_record()
	{
		...
		if(!empty($session)){ 
			if($record == 0){
				$this->load->view('hrms/admin/calendar/options/dialog_add_holiday', $data);
			} else if($record == 1){
				$this->load->view('hrms/admin/calendar/options/dialog_add_leave', $data);
			} else if($record == 2){
				$this->load->view('hrms/admin/calendar/options/dialog_add_travel', $data);
			} else if($record == 3){
				$this->load->view('hrms/admin/calendar/options/dialog_add_training', $data);
			} else if($record == 4){
				$this->load->view('hrms/admin/calendar/options/dialog_add_project', $data);
			} else if($record == 5){
				$this->load->view('hrms/admin/calendar/options/dialog_add_task', $data);
			} else if($record == 6){
				$this->load->view('hrms/admin/calendar/options/dialog_add_events', $data);
			} else if($record == 7){
				$this->load->view('hrms/admin/calendar/options/dialog_add_meetings', $data);
			} else if($record == 8){
				$this->load->view('hrms/admin/calendar/options/dialog_add_goal', $data);
			} else if($record == 9){
				$this->load->view('hrms/admin/calendar/options/dialog_add_events', $data);
			} else if($record == 10){
				$this->load->view('hrms/admin/calendar/options/dialog_add_events', $data);
