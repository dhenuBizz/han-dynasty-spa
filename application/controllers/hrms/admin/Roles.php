...
class Roles extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if($user[0]->user_role_id==1) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/roles/role_list", $data, TRUE);
    ...
    public function role_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/roles/role_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function read()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/roles/dialog_role', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
