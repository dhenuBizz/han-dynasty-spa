...
class Overtime_request extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('401',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/timesheet/overtime_request", $data, TRUE);
	...
	public function update_attendance_add() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/timesheet/dialog_overtime_request', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
	...
	// get company > employees
	 public function get_update_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/timesheet/get_request_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
