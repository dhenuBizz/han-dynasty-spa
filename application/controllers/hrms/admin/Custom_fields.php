...
class Custom_fields extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('393',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("hrms/admin/custom_fields/custom_fields_list", $data, TRUE);
    ...
    public function custom_fields_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/custom_fields/custom_fields_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	...
	public function read_info()
	{
       ...
       // $data['all_countries'] = $this->xin_model->get_countries();
		...
		if(!empty($session)){ 
			$this->load->view('hrms/admin/custom_fields/dialog_custom_fields', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
