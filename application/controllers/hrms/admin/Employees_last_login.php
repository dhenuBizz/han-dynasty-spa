	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('22',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/last_login/last_login_list", $data, TRUE);
    ...
    public function last_login_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/last_login/last_login_list", $data);
		} else {
			redirect('hrms/admin/');
		}
		...
		if(in_array('202',$role_resources_ids)) {
			$emp_link = '<a href="'.site_url().'hrms/admin/employees/detail/'.$r->user_id.'" data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'">'.$r->employee_id.'</a>';
		} else {
