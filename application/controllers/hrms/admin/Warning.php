...
class Warning extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('20',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/warning/warning_list", $data, TRUE);
	 ...
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/warning/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function get_employees_warning() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/warning/get_employees_warning", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
    ...
    public function warning_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/warning/warning_list", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
