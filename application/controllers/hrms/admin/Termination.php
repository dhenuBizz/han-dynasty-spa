...
class Termination extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('21',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/termination/termination_list", $data, TRUE);
    ...
    public function termination_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/termination/termination_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/termination/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function read()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/termination/dialog_termination', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
