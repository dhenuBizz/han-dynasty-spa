...
class Trainers extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('56',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/trainers/trainer_list", $data, TRUE);
    ...
    public function trainer_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/trainers/trainer_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function read()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/trainers/dialog_trainer', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
