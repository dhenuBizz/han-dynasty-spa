...
class Payroll extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function templates()
     {
		...
		if(in_array('34',$role_resources_ids)) {
			if(!empty($session)){
				$data['subview'] = $this->load->view("hrms/admin/payroll/templates", $data, TRUE);
	 ...
	 public function generate_payslip()
     {
		...
		if(in_array('36',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/payroll/generate_payslip", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	 ...
	 public function payment_history()
     {
		...
		if(in_array('37',$role_resources_ids)) {
			if(!empty($session)){
				$data['subview'] = $this->load->view("hrms/admin/payroll/payment_history", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	 ...
	 public function payslip_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/payroll/generate_payslip", $data);
		} else {
			redirect('hrms/admin/');
		}
        ...
        foreach($payslip->result() as $r) {
			...
			$full_name = '<a target="_blank" class="text-primary" href="'.site_url().'hrms/admin/employees/detail/'.$r->user_id.'">'.$emp_name.'</a>';
				...
				if($system[0]->is_half_monthly==1){
					...
					if($payment_check->num_rows() > 1) {
						//foreach($payment_last as $payment_half_last){
							...
							$view_url = site_url().'hrms/admin/payroll/payslip/id/'.$make_payment[0]->payslip_key;
							...
							$mpay = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_payroll_view_payslip').'"><a href="'.$view_url.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-arrow-circle-right"></span></button></a></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'hrms/admin/payroll/pdf_create/p/'.$make_payment[0]->payslip_key.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span>';
	...
	public function payroll_template_read()
	{
		...
		if(!empty($session)){ 
			$this->load->view('hrms/admin/payroll/dialog_templates', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
	...
	public function pay_salary()
	{
		...
		if(!empty($session)){ 
			$this->load->view('hrms/admin/payroll/dialog_make_payment', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
	...
	// hourly_list > templates
	 public function payment_history_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/payroll/payment_history", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	...
	// payment history
	 public function payslip()
     {
		...
		if(is_null($result)){
			redirect('hrms/admin/payroll/generate_payslip');
		}
		$p_method = '';
		...
		if($result[0]->payslip_type=='hourly'){
			$data['subview'] = $this->load->view("hrms/admin/payroll/hourly_payslip", $data, TRUE);
		} else {
			$data['subview'] = $this->load->view("hrms/admin/payroll/payslip", $data, TRUE);
		}
		$this->load->view('hrms/admin/layout/layout_main', $data); //page load
		} else {
			redirect('hrms/admin/');
	 ...
	 public function pdf_createv2(){
		...
		if(null!=$this->uri->segment(5) && $this->uri->segment(5)=='p') {
		// -----------------------------------------------------------------------------		
		$tbl2 = '';
		// -----------------------------------------------------------------------------
		$txt = 'Payslip Details';
	...
	// get company > employees
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/payroll/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	...
	public function make_payment_view()
	{
       ...
       // $data['all_countries'] = $this->xin_model->get_countries();
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/payroll/dialog_payslip', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
	...
	// get company > locations
	 public function get_company_plocations() {
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				...
				$this->load->view("hrms/admin/payroll/get_company_plocations", $data);
			} else {
				redirect('hrms/admin/');
			}
		}
	 ...
	 public function get_location_pdepartments() {
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				$data = $this->security->xss_clean($data);
				$this->load->view("hrms/admin/payroll/get_location_pdepartments", $data);
			} else {
				redirect('hrms/admin/');
			}
		}
	 ...
	 public function get_department_pdesignations() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/payroll/get_department_pdesignations", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
