...
class Department extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('3',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("hrms/admin/department/department_list", $data, TRUE);
	 ...
	 public function sub_departments()
     {
		...
		if(in_array('3',$role_resources_ids)) {
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("hrms/admin/department/sub_department_list", $data, TRUE);
			$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
    ...
    public function sub_department_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/department/sub_department_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function department_list() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/department/department_list", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function get_employees() {
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				...
				$this->load->view("hrms/admin/department/get_employees", $data);
			} else {
				redirect('hrms/admin/');
			}
		}
	 ...
	 public function get_company_locations() {
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				$data = $this->security->xss_clean($data);
				$this->load->view("hrms/admin/department/get_company_locations", $data);
			} else {
				redirect('hrms/admin/');
			}
		}
	 ...
	 public function read()
	{
		...
		if(is_numeric($keywords[0])) {
			...
			if(!empty($session)){ 
				$this->load->view('hrms/admin/department/dialog_department', $data);
			} else {
				redirect('hrms/admin/');
			}
		}
	...
	public function read_sub_record()
	{
		...
		if(is_numeric($keywords[0])) {
			...
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view('hrms/admin/department/dialog_sub_department', $data);
			} else {
				redirect('hrms/admin/');
			}
		}
