...
class Settings extends MY_Controller {
	...
	public function output($Return=array()){
     ...
     {	
		...
		if(in_array('60',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/settings", $data, TRUE);
     ...
     {	
		...
		if(in_array('118',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/payment_gateway_settings", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	 ...
	 public function database_backup()
     {
		...
		if(in_array('62',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/database_backup", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	 ...
	 public function modules()
     {
		...
		if(in_array('93',$role_resources_ids)) {	
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/modules", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	  ...
	  public function database_backup_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/settings/database_backup", $data);
		} else {
			redirect('hrms/admin/');
		}
		...
		$data[] = array('<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download').'"><a href="'.site_url().'hrms/admin/download?type=dbbackup&filename='.$r->backup_file.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->backup_id . '"><span class="fa fa-trash"></span></button></span>',
			$r->backup_file,
			$created_at
		);
      }
	...
	public function email_template() {
		...
		if(in_array('63',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/email_template", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
	...
	// email templates > list
	  public function email_template_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/settings/email_template", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	  ...
	  public function security_level_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/settings/constants", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	...
	public function read_tempalte()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/settings/dialog_email_template', $data);
		} else {
			redirect('hrms/admin/');
		}
	} 
	...
	public function password_read()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/settings/dialog_constants', $data);
		} else {
			redirect('hrms/admin/');
		}
	} 
	...
	public function constants()
     {
		...
		if(in_array('61',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/settings/constants", $data, TRUE);
				$this->load->view('hrms/admin/layout/layout_main', $data); //page load
			} else {
				redirect('hrms/admin/');
			}
