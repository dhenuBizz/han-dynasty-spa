...
class Employee_exit extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('23',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/exit/exit_list", $data, TRUE);
    ...
    public function exit_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/exit/exit_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/exit/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	 ...
	 public function read()
	{
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view('hrms/admin/exit/dialog_exit', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
