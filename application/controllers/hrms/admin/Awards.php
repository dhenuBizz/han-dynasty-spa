...
class Awards extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		if(in_array('14',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("hrms/admin/awards/award_list", $data, TRUE);
    ...
    public function award_list()
     {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/awards/award_list", $data);
		} else {
			redirect('hrms/admin/');
		}
	 ...
	 public function get_employees() {
		...
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("hrms/admin/awards/get_employees", $data);
		} else {
			redirect('hrms/admin/');
		}
		// Datatables Variables
	...
	public function read()
	{
		...
		if(!empty($session)){ 
			$this->load->view('hrms/admin/awards/dialog_award', $data);
		} else {
			redirect('hrms/admin/');
		}
	}
